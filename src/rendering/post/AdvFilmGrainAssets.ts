export const ADV_FILM_GRAIN_ASSET_SCHEMA =
  "haneoka.vega.renderer-asset/v1" as const;
export const ADV_FILM_GRAIN_ASSET_PREFIX =
  "haneoka.renderer-three/film-grain" as const;
...
export type AdvBuiltinFilmGrainType =
  | 0
  | 1
  | 2
  | 3
...
export type AdvFilmGrainPresetName =
  (typeof ADV_FILM_GRAIN_PRESET_NAMES)[number];
...
const BUILTIN_FILM_GRAIN_REQUESTS = Object.freeze(
  ADV_FILM_GRAIN_PRESET_NAMES.map(
    (preset, builtinType): AdvBuiltinFilmGrainAssetRequest =>
      Object.freeze({
        kind: "vega-renderer-asset",
        assetKey:
          `${ADV_FILM_GRAIN_ASSET_PREFIX}/${builtinType}` as AdvBuiltinFilmGrainAssetRequest["assetKey"],
...
export const advBuiltinFilmGrainAssetRequest = (
  type: number,
): AdvBuiltinFilmGrainAssetRequest | null => {
  const index = Math.trunc(Number(type));
  return index >= 0 && index < BUILTIN_FILM_GRAIN_REQUESTS.length
    ? BUILTIN_FILM_GRAIN_REQUESTS[index]!
    : null;
};
...
export const planAdvFilmGrainTexture = (
  type: number,
  customTextureReference: unknown,
): AdvFilmGrainTexturePlan => {
  const index = Math.trunc(Number(type));
...
export const resolveAdvFilmGrainTextureBinding = <TextureValue>(
  ...
  const texture =
    plan.reference === null ? null : resolve(plan.reference);
  return Object.freeze({
    reference: plan.reference,
    texture,
    activeWhenRequested:
      plan.proceduralFallback || texture !== null,
    usesProceduralFallback:
      plan.proceduralFallback && texture === null,
  });
};

export const isAdvBuiltinFilmGrainAssetRequest = (
  value: unknown,
): value is AdvBuiltinFilmGrainAssetRequest => {
  ...
  return (
    ...
    advBuiltinFilmGrainAssetRequest(candidate.metadata.builtinType ?? -1)
