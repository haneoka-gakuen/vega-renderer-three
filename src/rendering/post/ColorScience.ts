...
export type AdvLmsCoefficients = readonly [long: number, medium: number, short: number];
 ...
 */
export function cieXyToCat02Lms(xValue: number, yValue: number): AdvLmsCoefficients {
  const x = finite(xValue, D65_X);
  const y = Math.max(MIN_CHROMATICITY, finite(yValue, D65_Y));
  const xyz = [x / y, 1, (1 - x - y) / y] as const;
  return CAT02.map((row) => row[0] * xyz[0] + row[1] * xyz[1] + row[2] * xyz[2]) as unknown as AdvLmsCoefficients;
}
 ...
 */
export function advColorBalanceLms(temperatureValue: number, tintValue: number): AdvLmsCoefficients {
  ...
  const daylightDelta = cieDaylightLocusY(x) - cieDaylightLocusY(D65_X);
