import type { AdvFrameEntry, AdvFrameNumericRange, AdvFrameParticleLayer } from "@haneoka/vega/renderer-kit";
...
export interface AdvRainFrameSnapshot {
  readonly version: 1;
  ...
  readonly particles: readonly Readonly<RainParticle>[];
}
...
function resolveLayer(frame: AdvFrameEntry, layer: AdvFrameParticleLayer): ResolvedRainLayer {
  ...
  return {
    ...
    shapeLength: 2 * Math.abs(finite(layer.shape?.scale?.x, 1)) * Math.abs(finite(layer.shape?.radius, 1)),
...
export class AdvRainSimulation {
  ...
  private savedSnapshot: AdvRainFrameSnapshot | undefined;
  revision = 0;
  get liveParticles(): readonly Readonly<RainParticle>[] {
    return this.particles;
  }
  get capacity(): number {
    return this.layers.reduce((sum, layer) => sum + layer.maxParticles, 0);
  }
  constructor(frame: AdvFrameEntry, seed?: number) {
    ...
    this.seed = seed === undefined ? randomSeed[0] || fallbackSeed : seed >>> 0 || 1;
  }
  setViewport(width: number, height: number): void {
    ...
    if (previousWidth > 0 && previousHeight > 0 && nextWidth > 0 && nextHeight > 0) {
      ...
      for (const particle of this.particles) {
        particle.x = nextWidth / 2 + (particle.x - previousWidth / 2) * scale;
        particle.y = nextHeight / 2 + (particle.y - previousHeight / 2) * scale;
        particle.vx *= scale;
        particle.vy *= scale;
  ...
  snapshot(): AdvRainFrameSnapshot {
    return (this.savedSnapshot ??= {
      ...
      height: this.height,
      seed: this.seed,
      remainder: this.simulationRemainder,
  ...
  restore(snapshot: AdvRainFrameSnapshot): void {
    if (
      snapshot.version !== 1 ||
      snapshot.accumulators.length !== this.layers.length ||
      snapshot.particles.length > this.layers.reduce((sum, layer) => sum + layer.maxParticles, 0) ||
      ![snapshot.width, snapshot.height, snapshot.seed, snapshot.remainder, ...snapshot.accumulators].every(
        Number.isFinite,
      ) ||
      snapshot.width < 0 ||
      snapshot.height < 0 ||
      snapshot.remainder < 0 ||
      snapshot.remainder > MAX_CATCH_UP_SECONDS ||
      snapshot.seed < 0 ||
      snapshot.seed > 0xffffffff ||
      snapshot.particles.some(
        (particle) =>
          ![
            particle.layerIndex,
            particle.x,
            ...
            particle.lifetime,
            particle.age,
            particle.rotationCos,
            particle.rotationSin,
          ].every(Number.isFinite) ||
          !Number.isInteger(particle.layerIndex) ||
          particle.layerIndex < 0 ||
          particle.layerIndex >= this.layers.length,
      )
    ) {
      throw new TypeError("Invalid rain frame snapshot");
    }
    this.recycleParticles();
    const scale = snapshot.width > 0 ? this.width / snapshot.width : 1;
    for (const saved of snapshot.particles) {
      const particle = this.particlePool.pop() ?? ({} as RainParticle);
      Object.assign(particle, saved);
      if (this.width !== snapshot.width || this.height !== snapshot.height)
        Object.assign(particle, {
          x: this.width / 2 + (saved.x - snapshot.width / 2) * scale,
          y: this.height / 2 + (saved.y - snapshot.height / 2) * scale,
          vx: saved.vx * scale,
          vy: saved.vy * scale,
          width: saved.width * scale,
          height: saved.height * scale,
        });
      this.particles.push(particle);
    }
    this.seed = snapshot.seed >>> 0;
    this.savedSnapshot = undefined;
    this.simulationRemainder = snapshot.remainder;
    this.prewarmed = snapshot.prewarmed;
    this.revision++;
    this.accumulators.splice(0, this.accumulators.length, ...snapshot.accumulators);
  }

  update(deltaSeconds: number): void {
    if (this.width <= 0 || this.height <= 0 || deltaSeconds <= 0) return;
    this.savedSnapshot = undefined;
    this.prewarm();
    this.simulationRemainder = Math.min(
      MAX_CATCH_UP_SECONDS,
      this.simulationRemainder + Math.max(0, finite(deltaSeconds)),
    );
  ...
  private spawn(layerIndex: number, age = 0): boolean {
    ...
    // SingleSidedEdge spans [-radius, +radius] on local X. Scale Y does not
    // widen its emission region; particles start on the edge's Y origin.
    ...
    const width = Math.max(0, this.randomRange(layer.sizeX) * PARTICLE_TO_CANVAS * canvasScale);
    const height = Math.max(0, this.randomRange(layer.sizeY) * PARTICLE_TO_CANVAS * canvasScale);
